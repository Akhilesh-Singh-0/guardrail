...
export const updateUserLimits = async (
  ...
  if (
    ...
    throw new AppError(
      'Provide at least one limit value',
      400,
      'INVALID_BODY'
    )
  }
  ...
  if (dailyLimitUSD !== undefined) {
    data.dailyLimitUSD = new Decimal(
      dailyLimitUSD
    )
  }

  if (monthlyLimitUSD !== undefined) {
    data.monthlyLimitUSD = new Decimal(
      monthlyLimitUSD
    )
  }
  ...
  res.status(200).json({
    ...
    limits: {
      dailyLimitUSD:
        updated.dailyLimitUSD.toString(),

      monthlyLimitUSD:
        updated.monthlyLimitUSD.toString(),
